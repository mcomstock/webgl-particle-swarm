...
void main() {
    ...
    float scale = 1.0 / 12.9; // Scale parameter from paper to convert time from AU to ms

    float k = particles_1.r * scale;
    float a = particles_1.g;
    float b = particles_1.b;
    float eps0 = particles_1.a * scale;

    float mu1 = particles_2.r;
    float mu2 = particles_2.g;
    ...
    for (int step_count = 1; step_count <= num_steps; ++step_count) {
        ...
        du = k*u*(1.0-u)*(u-a) - u*v + stim;
        dv = (eps0 + mu1*v / (u + mu2)) * (-v - k*u*(u - b- 1.0));
    ...
    state_out_texture_0 = vec4(u, v, 0.0, 0.0);
}
